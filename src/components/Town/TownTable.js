...
const TownTable = () => {
    ...
    const {datatown, handleSubmit,Townset,  open , setOpen,handleUpdate, handleDelete,  onChange, handleClose, handleClickOpen} = useContext(TownContext)
    //light or dark mode
    ...
    return (
        <div className={user.marg}>
             <Typography align= 'center' sx={{fontSize: '39px'}} size = "large" >Town</Typography>
            ...
            <Box sx={{ '& > :not(style)': { m: 1 } }} >
                ...
                <TownModal open={open} handleClose={handleClose} data={Townset}  onChange={onChange} handleSubmit={handleSubmit} />
        </div>
    );
}
