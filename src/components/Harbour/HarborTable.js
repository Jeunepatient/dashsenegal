...
import { HarborContext } from './HarborContext';
import { Typography } from '@mui/material';


const HarborTable = () => {
    //destructuring the needful data from the context
    const {datarow, setData, handleSubmit,Harbor, SetHarbor,  open , setOpen,handleUpdate, handleDelete, initialvalues, onChange, handleClose, handleClickOpen} = useContext(HarborContext)
    //light or dark mode
    ...
    return (
        <div className={user.marg}>
            ...
            <Typography align= 'center' sx={{fontSize: '39px'}} size = "large" >Harbor</Typography>
            ...
            <Box sx={{ '& > :not(style)': { m: 1 } }} >
                ...
                <HarborModal open={open} handleClose={handleClose} data={Harbor}  onChange={onChange} handleSubmit={handleSubmit} />
        </div>
    );
}
