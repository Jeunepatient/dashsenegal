...
const CountryTable = () => {
    ...
    const {dataCountry, handleSubmit,countryset,  open , handleUpdate, handleDelete,  onChange, handleClose, handleClickOpen} = useContext(CountryContext)
    //light or dark mode
    ...
    return (
        ...
        <Typography align= 'center' sx={{fontSize: '39px'}} size = "large" >Countries</Typography>
            ...
            <Box sx={{ '& > :not(style)': { m: 1 } }} >
                ...
                <CountryModal open={open} handleClose={handleClose} data={countryset}  onChange={onChange} handleSubmit={handleSubmit} />
        </div>
    );
}
